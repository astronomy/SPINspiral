...
//printf("lengthLAL2 = %d\n", lengthLAL);
 ...
 // double n_J0[3] = {cos(par->phiJ0)*sthJ0,sin(par->phiJ0)*sthJ0,cthJ0};
  ...
  facvec(n_J0,-sthJ0,tvec1);                                                                                       //tvec1 = -J0^*cos(theta_J0)
  addvec(n_z,tvec1,tvec2);                                                                                               //tvec2 = n_z - J0^*cos(theta_J0)
  //facvec(tvec2,1.0/sthJ0,cvec1);                                                                                         //cvec1 = (n_z - J0^*cos(theta_J0))/sin(theta_J0)
...
for (i=0; i<length; ++i){
  ...
  //  if ((omega_orb>=omega_low) && (terminate==0)) {  // After source comes into window, before tc and before frequency reaches its maximum  Careful, if t<0, omega = nan!!!
    if ((omega_orb>=omega_low) && (terminate==0)) {  
...
//	x1     = 2.0*exp(5.0*c3rd*log(Mc))/D_L;
	...
	ifo[ifonr]->FTin[i] = Fplus*hplusLAL[i-i1] + Fcross*hcrossLAL[i-i1];
	
//	printf("%d\t%.6e\t%.6e\n", i, ifo[ifonr]->FTin[i], wave[1][i-i1+1]);
	//****************************************************************************************************************************************************************************
