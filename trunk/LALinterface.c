...
//	printf("k = %10.20f\n", par->kappa);
	...
	*/
    ...
    injParams.distance = exp(par->logdl);//d_L;
//	
   ...
   // printf("phi0 ?= %10.20f\n", Lphi02);
    ...
    injParams.inclination = acos(LdotN);//*****************************************************************************************bornes de l'inclinaison a verifier**************
//	
    //injParams.polarization = locpolar;
	//printf("polarization = %10.20f\n", injParams.polarization);

    injParams.spin1x = n_S[0];
    injParams.spin1y = n_S[1];
    injParams.spin1z = n_S[2];
//	
